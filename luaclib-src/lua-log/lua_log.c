...
void rollfile(int loglevel) {
...
#ifdef _MSC_VER
            if (mkdir(inst.dirname[loglevel]) == -1)
#else
    ...
    while (1) {
        ...
        snprintf(filename, sizeof(filename), "%s/%s", inst.dirname[loglevel],
                 get_log_filename(inst.use_basename[loglevel], inst.basename, inst.roll_type[loglevel], index++,
                                  loglevel));
        ...
        if (inst.handle[loglevel] == NULL) {
            ...
            if (inst.roll_type[loglevel] == 0)
                ...
                inst.roll_day[loglevel] = tm.tm_mday;
...
static inline void *worker_func(void *p) {
    ...
    while (inst.running) {
        ...
        if (inst.buffers.head == NULL) {
            ...
            //tv.tv_usec  微秒    ts.tv_nsec  纳秒
#endif
#ifndef _MSC_VER
            ts.tv_sec += inst.flush_interval;
            pthread_cond_timedwait(&inst.cond, &inst.mutex, &ts);
#else
            sleep(inst.flush_interval * 10);
            int res = pthread_cond_timedwait(&inst.cond, &inst.mutex, &ts);
            if (res == 0) {
//                printf("Consumer: Data received and consumed.\n");
...
//            pthread_cond_wait(&inst.cond, &inst.mutex);
        ...
        for (node = buffers_to_write.head; node != NULL; node = node->next) {
            for (int i = 0; i < node->size; i++) {
                ...
                if (inst.handle[lv]) {
                    // 新的一小时、天，滚动日志
                    if ((inst.roll_type[lv] == 0 && inst.roll_hour[lv] >= 0 && inst.roll_hour[lv] != tm.tm_hour)
                        || (inst.roll_type[lv] == 1 && inst.roll_day[lv] > 0 && inst.roll_day[lv] != tm.tm_mday))
                        rollfile(lv);
#ifndef _MSC_VER
                    fwrite_unlocked(node->message[i].data, 1, node->message[i].len, inst.handle[lv]);
#else

                    fwrite(node->message[i].data, 1, node->message[i].len, inst.handle[lv]);
#endif
                    inst.written_bytes[lv] += node->message[i].len;
                    need_flush[lv] = 1;
                }
            }
...
static void log_exit() {
    inst.running = 0;
    pthread_cond_signal(&inst.cond); // 唤醒线程,写完剩下的内容
