 ...
 * @type : c
 * @author : copy
 * @created : 2023-11-29 14:35:13
 * @Last Modified time: 2023-11-29 14:35:13
 * @brief : A*寻路
			 ...
			 float *dis, int uindex)
{
	...
	lua_pushinteger(L, uindex);
	ret = lua_pcall(L, 7, LUA_MULTRET, 0);
	if (ret != 0) {
		lua_settop(L, top);
		return -1;
	}
...
static void path_neighbors(ASNeighborList neighbors, void *node, void *context)
{
	...
	for (i=-1; i<=1; i++) {
		for (j=-1; j<=1; j++) {
			...
			if (get_neighbors(L, map->ref, w, h, sx, sy, tx, ty, &dis, map->uindex) == 0) {
...
static int lua__new_map(lua_State *L)
{
	...
	if (!lua_isfunction(L, 3)) {
		luaL_error(L, "3 argument not function type");
		return 0;
	}
	...
	*pmap = map;
	...
	map->uindex = (int)luaL_checkinteger(L, 4);
...
static int lua__path(lua_State *L)
{
	...
	lua_pushnumber(L, ASPathGetCount(path));
	ASPathDestroy(path);
	return 2;
}
